...
func GetWorldTypeStorageSeedThatHasSmallestFloatingPepperSeed(c echo.Context) error {
	...
	err := config.DB.Where("type = ? AND float_pepper_seed_count > ? AND sl_owner = ? AND ? - last_accessed > ?", "storage_seed", 0, "notfound", currentTime, 45).Order("float_pepper_seed_count asc").First(&existingWorld).Error
	if err != nil {
		errorSecond := config.DB.Where("type = ? AND float_pepper_seed_count > ? AND ? - last_accessed > ?", "storage_seed", 0, currentTime, 45).Order("float_pepper_seed_count asc").First(&existingWorld).Error
		if errorSecond != nil {
			return c.JSON(http.StatusInternalServerError, utils.ErrorResponse("Failed to retrieve world"))
		} else {
			updatedWorld = existingWorld
